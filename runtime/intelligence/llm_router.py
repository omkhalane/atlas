...
class LLMRouter:
    ...
    def route(self, intent_result: Dict[str, Any]) -> ReasoningSession:
        ...
        # Case 2: Local-only (easy/medium, no cloud needed)
        if not needs_cloud and difficulty in ("easy", "medium"):
            logger.info(f"Router: Local LLM → {self.local_llm.selected_model if self.local_llm else 'N/A'}")
            return ReasoningSession("local", local_llm=self.local_llm, openrouter=self.openrouter)

        # Case 3: Cloud routing by intent type
        intent_prefix = intent.split(".")[0] if "." in intent else intent
        cloud_model = INTENT_MODEL_MAP.get(intent_prefix)
        if cloud_model:
            logger.info(f"Router: Cloud → {cloud_model} (intent match: {intent_prefix})")
        else:
            logger.info(f"Router: Cloud → OpenRouter fallback chain")
