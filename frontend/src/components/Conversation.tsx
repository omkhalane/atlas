...
const CodeBlock = ({ node, inline, className, children, ...props }: any) => {
  ...
  if (!inline && match) {
    return (
      <div className="relative group rounded-xl overflow-hidden my-4 border border-[var(--color-border-subtle)] bg-[#0d0d0d]">
    ...
    return (
      <div className={`w-full flex ${ev.type === 'user_prompt' ? 'justify-end' : 'justify-start'} py-2`}>
        <div className="max-w-[800px] w-full flex flex-col">
          {ev.type === 'user_prompt' && (
            <div className="self-end bg-[var(--color-bg-hover)] text-[var(--color-text-primary)] px-5 py-3.5 rounded-[20px] rounded-br-[6px] max-w-[80%] text-[15px] border border-[var(--color-border-subtle)] shadow-sm">
              {ev.content}
            </div>
          )}
          
          {ev.type === 'thought' && (
            <div className="flex items-start gap-3 text-[var(--color-text-secondary)] text-[14px] ml-4 my-1 bg-[var(--color-bg-background)]">
               <Sparkles size={16} className="text-[var(--color-text-primary)] shrink-0 mt-0.5" /> 
               <div className="leading-relaxed prose prose-invert max-w-none prose-p:my-0 prose-pre:my-0 text-[var(--color-text-secondary)]">
          ...
          {ev.type === 'action' && (
            <div className="flex flex-col gap-2 w-full ml-10 my-2">
              <div className="flex items-center gap-2 text-[13px] font-medium text-[var(--color-text-secondary)] bg-[var(--color-bg-panel)] w-max px-3 py-1.5 rounded-lg border border-[var(--color-border-subtle)]">
                {getIconForCapability(ev.action || '')}
                <span className="capitalize">{ev.action}</span>
                <span className="w-1.5 h-1.5 rounded-full bg-[var(--color-accent)] animate-pulse ml-2" />
                <span className="text-[var(--color-text-tertiary)]">Running...</span>
              </div>
            </div>
          )}
          ...
          {ev.type === 'finish' && (
            <div className="ml-10 text-[var(--color-success)] text-sm flex items-center gap-2 mt-4 mb-8">
              <span className="w-2 h-2 rounded-full bg-[var(--color-success)] shadow-[0_0_10px_rgba(34,197,94,0.5)]" />
              Task Completed
            </div>
          )}
          ...
          {ev.type === 'message' && (
            <div className="prose prose-invert max-w-none ml-10 text-[15px] leading-relaxed prose-pre:bg-transparent prose-pre:p-0 prose-pre:m-0 text-[var(--color-text-primary)]">
