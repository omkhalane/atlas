...
export const PromptBox: React.FC<{ convId?: string }> = ({ convId }) => {
  ...
  const handleExecute = async () => {
    ...
    setIsExecuting(true);
    try {
      let targetConvId = convId || activeConversationId;
      if (!targetConvId) {
        targetConvId = useStore.getState().createConversation(null);
        // We just created a new conversation. If the current active tab is a generic chat tab, we should bind it.
        const state = useStore.getState();
        const pane = state.panes.find(p => p.id === state.activePaneId);
        if (pane && pane.activeTabId) {
          const tab = pane.tabs.find(t => t.id === pane.activeTabId);
          if (tab && tab.type === 'chat' && !tab.data?.convId) {
             const newPanes = state.panes.map(p => {
               if (p.id === pane.id) {
                 return {
                   ...p,
                   tabs: p.tabs.map(t => t.id === tab.id ? { ...t, data: { ...t.data, convId: targetConvId } } : t)
                 };
               }
               return p;
             });
  ...
  return (
    <motion.div 
      ...
      className="absolute bottom-8 left-1/2 -translate-x-1/2 w-full max-w-[860px] px-4 z-40 flex flex-col gap-3"
    >
      {isExecuting && (
         <motion.div 
           initial={{ opacity: 0, y: 10 }}
           animate={{ opacity: 1, y: 0 }}
           className="w-full bg-[var(--color-bg-panel)] border border-[var(--color-border-active)] rounded-xl p-3 shadow-lg flex flex-col gap-2"
         >
            <div className="flex items-center justify-between">
               <div className="flex items-center gap-2">
                 <span className="w-2 h-2 rounded-full bg-[var(--color-accent)] animate-pulse" />
                 <span className="text-[13px] font-medium text-[var(--color-text-primary)]">Agent Status: Running</span>
               </div>
               <span className="text-[12px] font-mono text-[var(--color-text-tertiary)]">00:45</span>
            </div>
            <div className="text-[12px] text-[var(--color-text-secondary)]">Task: Executing plan...</div>
            <div className="flex gap-2 mt-1">
               <button onClick={() => setIsExecuting(false)} className="px-3 py-1.5 bg-[var(--color-danger-transparent)] text-[var(--color-danger)] text-[11px] rounded border border-red-900/30 hover:bg-red-900/40 transition-colors">
                  Cancel Task
               </button>
            </div>
         </motion.div>
      ...
      >
        ...
        <div className="flex px-3 pt-2 items-start gap-2">
          <span className="text-[var(--color-text-tertiary)] font-mono text-[16px] mt-1 shrink-0">{'>'}</span>
          <textarea 
            ref={textareaRef}
            value={taskInput}
        ...
        <div className="flex items-center justify-between px-2 pb-1 pt-2">
          ...
          <div className="flex items-center gap-2">
            ...
            {!isExecuting && (
               <span className="text-[10px] font-mono text-[var(--color-text-tertiary)] absolute -top-5 right-2 opacity-0 group-hover:opacity-100 transition-opacity">Ctrl+Enter</span>
            )}
          </div>
        </div>
      </div>
